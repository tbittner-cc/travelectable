...
def hotels():
    ...
    return render_template('hotel_search_results.html',
                           ...
                           amenities=utilities.get_amenities(
                               session['destination'][0]))


@app.route("/hotel-sort", methods=['GET', 'POST'])
def hotel_sort():
    hotels = utilities.get_hotels_with_amenities(session['destination'],
                                                 session['filtered_amenities'])
    add_lead_rates(hotels, session['dates'])
...
def add_amenity():
    ...
    if request.form['amenities_input'] != '' and request.form[
            'amenities_input'] in all_utilities:
        fa.append(request.form['amenities_input'])
        session['filtered_amenities'] = fa
    resp = make_response(
        render_template('filtered_amenities.html',
...
def hotel_details():
    ...
    hotel = utilities.get_hotel_details(session['destination'],
                                        session['dates'], hotel_id,
                                        is_winter_rate)
...
def search():
    ...
    selected_locations = utilities.get_selected_locations(
        location_queries, locations)

    if location_queries['origin'] == '' and location_queries[
            'destination'] == '':
